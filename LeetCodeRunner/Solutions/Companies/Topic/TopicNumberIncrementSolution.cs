...
{
    internal class TopicNumberIncrementSolution : AbstractSolution<TopicNumberIncrementSolution.INPUT, int>
    {
        ...
        protected override IEnumerable<SolutionTestCase<INPUT, int>> GetTestCases()
        {
            return new SolutionTestCase<INPUT, int>[]
            {
                ...
                {
                    Input = new INPUT(920, 7),
                    ExpectedResult = 990
                },
                new SolutionTestCase<INPUT, int>()
                {
                    Input = new INPUT(100, 2),
                    ExpectedResult = 300
                }
            };
        }
        ...
        public record INPUT(int N, int K);

        protected override bool IsMatch(int v1, int v2) => v1 == v2;
        protected override int RunTestCase(INPUT data) => solution(data.N, data.K);
